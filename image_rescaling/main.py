...
def see_irn_example(x, y, z, x_recon_from_y, scale, see=True, save=True, wandb_log=False, wandb_step=-1, name=0, render_grid=True):
    ...
    if render_grid:
        fig = see_multiple_imgs(imgs, 2, 3,
            ...
            see=see, save=save,
            filename=f'/rds/user/mgm52/hpc-work/invertible-image-rescaling/output/outrgb_{int(time.time())}_{i}_{name}'
        )
        ...
        if save:
            ...
            filename_start = f'/rds/user/mgm52/hpc-work/invertible-image-rescaling/output/out_{int(time.time())}_{i}_{name}_'
